...
def login(driver, user):
    ...
    WebDriverWait(driver, 60).until(
        EC.presence_of_element_located((By.NAME, 'M_PORTAL_LOGIN_ACNT')))
    WebDriverWait(driver, 60).until(
        EC.presence_of_element_located((By.NAME, 'M_PW')))

    driver.find_element_by_name(
        'M_PORTAL_LOGIN_ACNT').send_keys(user['account'])
...
def select_course(driver, course, thread_name):
    ...
    while (count == 1) or ((course['id'], course['class']) not in selected):
        if total_rows == 1:
            ...
            classes = list(map(lambda x: x.text.split()[
                           3], driver.find_elements_by_css_selector('#DataGrid1 tbody tr')[1:]))
        ...
        selected = list(map(lambda x: tuple(x.text.split()[
                        1: 3]), driver.find_elements_by_css_selector('#DataGrid3 tbody tr')[1:]))
...
def parallel(account, password, course):
    ...
    with webdriver.Chrome(executable_path='./chromedriver.exe', options=options) as driver:
        try:
            ...
            print(
                f'{thread_name} ({course["course_id"]}): Account or Password Error')
...
if __name__ == '__main__':
    while True:
        ...
        for i in range(course_amount):
            threads.append(threading.Thread(target=parallel, args=(
                account, password, config['courses'][i]), name=f'Thread {i + 1}'))
            threads[i].start()
